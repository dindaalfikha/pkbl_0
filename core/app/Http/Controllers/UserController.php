...
class UserController extends Controller
{
    ...
    public function loanDetail($id)
    {
        ...
        $data['loan_left'] = PayLoanHistory::selectRaw('SUM(amount) as total')->where([
            ['id_user', Auth::user()->id],
            ['id_loan', $id]
        ])->first();
        $data['loan'] = Loan::whereId($id)->first();
        $data['pay_loan_history'] = PayLoanHistory::where([
            ['id_user', Auth::user()->id],
            ['id_loan', $id]
        ])->get();

        return view('user.load_detail', $data);
    }
    ...
    public function loansubmit(Request $request)
    {
        ...
        if($loan<1){
            if($request->hasFile('agunan')) {
                // $uniqueFileName = time() . '_' . Auth::user()->id . '.' . $request->file('agunan')->getClientOriginalExtension();
                $file = $request->file('agunan');
                ...
                return back()->with('alert', 'Harus menyertakan agunan');
            }
        }else{
    ...
    public function payloan($id, Request $request)
    {
        ...
        if($request->hasFile('agunan')) {
            if($request->amount <= $user->balance) {
                ...
                $loan_left = $loan->amount - $loan_payed->total;

                if($loan_left > 0 && $request->amount >= $loan_left){
                    ...
                    $loan->save();
                    $user->save();
                } else if($loan_left > 0 && $request->amount < $loan_left) {
                ...
                return back()->with('alert', 'Jumlah pembayaran tidak boleh lebih dari saldo yang tersedia');
            }
        }
    }  
